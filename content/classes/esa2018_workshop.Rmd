...
snb_lsm_wo_check <- filter(snb_lsm3, Material !="Check") %>% 
  mutate(Material = reorder(Material,lsmean)) # reorder by mean yield

snb_lsm_check <- filter(snb_lsm3, Material =="Check") %>%
  select(-Timing) ## remove Timing to force onto all panels

ggplot(data = snb_lsm_wo_check,
       mapping = aes(x = Material, y = lsmean)) + 
  geom_point() + 
  ...
  geom_errorbar(data = snb_lsm_check,
                mapping = aes(ymin = lower.CL, ymax = upper.CL), color="grey") + 
