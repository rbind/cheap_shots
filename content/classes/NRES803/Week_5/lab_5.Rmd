...
knitr::opts_chunk$set(echo = TRUE, tidy = TRUE, tidy.opts=list(arrow = TRUE, width.cutoff=60, warning=FALSE, message=FALSE))
lab.eval=TRUE 
...
```{r,eval=lab.eval, results=lab.results, fig.show=lab.results}
nd = data.frame(area=seq(0,9,0.01),isolation=median(isolation$isolation))
pred.p <- augment(isolation.2,newdata=nd,type.predict="link",se.fit=TRUE) %>%
