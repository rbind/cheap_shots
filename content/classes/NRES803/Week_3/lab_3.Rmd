...
```{r}
npars <- map_dbl(fits, ~length(coef(.x))+1)
```

Recall that the second argument to `map_dbl()` is a function. Here I've defined an *anonymous* function (it has no name) just to use here. If that looks intimidating – start with `coef(global)` and build out and you’ll see what the function does with a single fitted model object. Finally, use `data_frame()` to show all the results together.[^usedataframe]

[^usedataframe]: In fact, we could have done all in the individual calculations directly inside the call to `data_frame()`. In this case I needed to explain each line one by one, so that becomes awkward.

```{r}
data_frame(AIC=aic,k=npars,deltas=deltas,weights=weights)
```
...
```{r}
aic.table <- data_frame(model=1:16,AIC=aic,k=npars,deltas=deltas,weights=weights)
at2 <- aic.table %>%
  arrange(AIC) %>%
