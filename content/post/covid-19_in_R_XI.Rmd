...
Yes, the USA has more cases than anyone else in the world, but per capita we have fewer than
...
The USA will catch up to Germany on a per capita basis within the next few days. 

I've got state level data back and do some exploration with that. I hope the
data structure is a bit more stable going forward. [The state and province figures
are below the Full Data Nerd heading](#full_data_nerd), just skip over the giant code blocks. 
...
Note that the y axis is logarithmic. The x axis is days since the 100th case in each country.
...
I think it's worth reminding everyone that the red box is a *very* fuzzy estimate of
...
than beds at this stage. 

If you've read this far, [there's a plot of canadian provinces](#canada_by_province),
and [several plots of different american states](#back_to_america). Be sure to 
scroll down past the enormous blocks of code.

# Full Data Nerd {#full_data_nerd}
...
## Canada by Province {#canada_by_province}
...
## Back to America {#back_to_america}
...
have higher prevalence but longer doubling times. Which four states have the highest per capita prevalence?
...
That's cases per 100,000 population, so even New York is only just 0.2%. So just
by the numbers your chance of being sick is 2 out of 1000 right now in the worst
hit place in the country. Of course it isn't evenly spread as that calculation assumes.
