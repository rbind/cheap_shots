...
Sunday's numbers still inside prediction interval, but below expectations. 
That's good news. I've started adding more countries for comparison, to see
...
Much as yesterday, Sunday's new cases are inside the prediction intervals but
...
by the frequency of testing for COVID-19. This is starting to ramp up in the USA,
so we might see the rate of new cases go up as that happens.

If there is a "weekend effect" in reporting, it would look like
...
One of my FaceBook friends asked, "Does anyone known anyone that actually has coronavirus?" 
...
There are possible signs of improvement, but this next week will be critical to seeing 
...
the data and looking for a date prior to where all the countries case loads started accelerating.
...
# test it
# augment_gam(all_results$models[[1]], se.fit = TRUE, newdata = tibble(day = 14:20))
# seems to work
all_results <- all_confirmed_total %>%
  mutate(day = as.numeric(Date - ymd("2020-02-27"))) %>% # get day relative to Feb 27
  filter(Date > "2020-02-15", Date <= "2020-03-11") %>% # trim data to the dates we want
...
This post benefited from comments by Ramona Sky and Kelly Helm-Smith.