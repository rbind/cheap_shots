...
This graph gives the probability that a 95% one sided confidence limit will
exclude the target, given the true value of average bare ground. Now that
...
a null of $\bar{x} < 38$. Failure to reject is 1 - power. 
...
results <- crossing(n = c(20, 40, 60, 80, 100), 
  ...
  mutate(bg = severity + 38,
         ...
         failure = 1 - power)
ggplot(data = results,
       mapping = aes(x = bg, y = failure, color = n)) + 
  geom_point() + 
  geom_line() + 
  xlab("% Bare ground")
```
