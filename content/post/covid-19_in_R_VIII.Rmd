...
Bit of perspective on the testing issue -- [apparently California tested 17000 people so far, but 10000 of those
tests are still pending](https://apple.news/AU6X4O-5WT3G6JGsqcOet8Q)!
...
handle. To put those numbers in perspective, the thin white line just above the latest point from italy is
1000 cases / 100,000 population. That's 1%. New cases in Italy are nearly 1% of the population *per day*. 
...
ggplot(data = all_by_province,
  ...
  geom_text(data = filter(all_by_province, province == "California"),
            mapping = aes(y = 1.4*icu_beds/0.05), 
            x = ymd("2020-03-01"),
           label = "20 x # ICU beds / 100K",
           size = 2.5, hjust = "left")
...
A prominent quantitative ecologist (who can remain nameless) dismissed my efforts here because the 
exponential model magnifies the uncertainty making forecasts useless, and making it impossible to refute the
exponential model. In addition, many models can be 
consistent with exponential growth at the beginning,
but do a lousy job of forecasting (for example, [the logistic model](/post/covid-19_and_R_v/) scroll down).
It's true that the forecasts here have a large amount of uncertainty -- the forecast incident cases in
...
large day to day fluctuations in reported cases in Ontario. In addition, there is no guarantee that
the exponential model appears to work just because of this uncertainty. Look at the plot for Italy.
It was clear on March 13 that the exponential model was no longer valid in Italy. Honestly, I've 
...
out. There are huge well-funded teams[^3] crunching the numbers and advising decision makers, so 
nothing lost by me taking a break. Here are my lessons learned 

- Simple models can be useful for decision making. If I'd thought to do it, I could have predicted the
ICU crunch in New York 2 weeks ago. 

- Data are critical. The biggest weakness for the US predictions is the lack of testing to identify cases. 

- Crowdsourcing can work! The JHU data aggregation is now the de facto go to for everyone, even the US government[^4]. 
...
[^4]: A news article I looked at yesterday had a picture of the DHHS situation room in DC. The JHU
dashboard was on the wallscreen in the background.