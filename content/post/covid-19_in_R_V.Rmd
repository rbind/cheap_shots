...
That was way harder than I planned on. The goto function for non-linear models like this is `nls()`, but the estimation algorithms available there were struggling to find converged 
parameters. Package `drc` (dose response curve) gave me good results before so I tried that.
...
But that's not the reason to discard these models -- with the exception of the USA, all
...
except for Australia. What about the estimated asymptotic sizes? 
